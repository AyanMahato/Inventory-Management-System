...
function onOpen() {
  ...
  {var newItem2=['Entry ID','Item ID', 'Item Name','Employee ID', 'Quantity', 'Transaction','Date & Time'];
...
function addItem() {
  ...
  var sheet01=SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheets[0].getName());
...
function processFormDataTransactions(itemID,itemName,empId,quantity,trans) {
  ...
  {
    if (tdata[i][3] == empId && tdata[i][5]=='Borrow' && tdata[i][1]==itemID && trans=='Borrow') 
    {
      var bdate=tdata[i][6];
      var diff=Math.round((tdate.getTime() - bdate.getTime()) / (1000 * 3600 * 24));
      var rborrow='';
      for(var i = 0; i < data.length; i++) {
       if (data[i][0] == itemID) {
     ...
     if (diff<=rborrow) {
      Browser.msgBox("Employee cannot borrow this item before "+(rborrow-diff)+" Days");
      ind=1;
     }
     break;
    }
 ...
 if(ind==0){
  ...
  if(c==0)
  {
  var tID=(tdatarange.getLastRow()==1)?1:tdata[tdatarange.getLastRow()-1][0]+1;
  ...
  datarange.setValues(data);
  sourceSheet.getRange(1, 1, 1, sourceSheet.getLastColumn()).setFontWeight('bold');
  }
 }
}
